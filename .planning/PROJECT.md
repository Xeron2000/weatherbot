...
这是一个基于现有 `weatherbot` 代码库继续演进的 Polymarket 天气市场自动交易项目，当前仓库定位已经收口为 **YES-only**：只保留低价 YES 窄温区猎杀这条活跃策略路径。v1 先做模拟交易，但要把自动扫描、候选筛选、被动挂单、订单生命周期和持仓管理链路完整跑通，为后续实盘接入留好执行边界。
...
- [ ] 将当前方向性入场逻辑收口并固化为 **YES-only** 的低价窄温区挂单框架
- [ ] 支持面向挂单的执行模型：报价、排队、成交、撤单、超时失效与剩余订单管理
- [ ] 在 20+ 城市天气市场中持续扫描 YES 候选标的，并按 YES-only 风控与挂单语义执行定价和仓位控制
...
- 当前系统已经完成配置面与运行时的 **YES-only** 收口，但项目真相源仍需继续明确：当前活跃运行路径只剩 YES 候选、YES 挂单和 YES 持仓管理。
- 当前活跃策略核心是低价 YES：窄温区（如 52–53°F、38–39°F）在 0.1–2¢ 区间挂 YES，吃高赔率命中。
- 历史背景：仓库早期曾讨论过 NO 腿，但该路径已移除，不再属于当前活跃目标、默认配置或操作者执行语义。
